  ...
  return (
    <DashboardLayout>
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        ...
        <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">{documents.length === 0 ? <div className="p-8 text-center"><p className="text-sm font-medium text-slate-700">No research documents found.</p><p className="mt-1 text-sm text-slate-500">Documents will appear here after they are added.</p></div> : <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="border-b border-slate-200 bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Title</th><th className="px-4 py-3">Employee</th><th className="px-4 py-3">Category</th><th className="px-4 py-3">File</th><th className="px-4 py-3">Actions</th></tr></thead><tbody className="divide-y divide-slate-200">{documents.map((document) => <tr key={document.id} className="hover:bg-slate-50"><td className="px-4 py-4"><div className="font-medium text-slate-950">{document.title}</div><div className="mt-1 text-slate-500">{document.description}</div></td><td className="px-4 py-4 text-slate-600">{document.user.name}</td><td className="px-4 py-4 text-slate-600">{document.category}</td><td className="px-4 py-4"><a href={document.fileUrl} target="_blank" rel="noreferrer" className="text-sm font-medium text-slate-700 hover:text-slate-950">Open</a></td><td className="px-4 py-4">{document.user.id === sessionUser.id ? <Link href={`/research/${document.id}/edit`} className="text-sm font-medium text-slate-700 hover:text-slate-950">Edit</Link> : <span className="text-sm text-slate-400">View only</span>}</td></tr>)}</tbody></table></div>}</section>
