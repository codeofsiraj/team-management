      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        ...
        <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
          {entries.length === 0 ? <div className="p-8 text-center"><p className="text-sm font-medium text-slate-700">No tool usage entries found.</p><p className="mt-1 text-sm text-slate-500">Entries will appear here after they are added.</p></div> : (
            <div className="overflow-x-auto"><table className="w-full min-w-[940px] text-left text-sm"><thead className="border-b border-slate-200 bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Tool</th><th className="px-4 py-3">Employee</th><th className="px-4 py-3">Category</th><th className="px-4 py-3">Purpose</th><th className="px-4 py-3">Time</th><th className="px-4 py-3">Date</th><th className="px-4 py-3">Actions</th></tr></thead><tbody className="divide-y divide-slate-200">{entries.map((entry) => <tr key={entry.id} className="hover:bg-slate-50"><td className="px-4 py-4 font-medium text-slate-950">{entry.toolName}</td><td className="px-4 py-4 text-slate-600">{entry.user.name}</td><td className="px-4 py-4 text-slate-600">{entry.category}</td><td className="px-4 py-4 text-slate-600">{entry.purpose}</td><td className="px-4 py-4 text-slate-600">{entry.timeSpent}</td><td className="px-4 py-4 text-slate-600">{dateFormatter.format(entry.date)}</td><td className="px-4 py-4">{entry.user.id === sessionUser.id ? <Link href={`/tools/${entry.id}/edit`} className="text-sm font-medium text-slate-700 hover:text-slate-950">Edit</Link> : <span className="text-sm text-slate-400">View only</span>}</td></tr>)}</tbody></table></div>
          )}
        </section>
