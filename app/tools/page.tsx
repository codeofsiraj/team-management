      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        ...
        <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
          {entries.length === 0 ? <div className="p-8 text-center"><p className="text-sm font-medium text-slate-700">No tool usage entries found.</p><p className="mt-1 text-sm text-slate-500">Entries will appear here after they are added.</p></div> : (
            <div className="overflow-x-auto"><table className="w-full min-w-[940px] text-left text-sm"><thead className="border-b border-slate-200 bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Tool</th><th className="px-4 py-3">Employee</th><th className="px-4 py-3">Category</th><th className="px-4 py-3">Purpose</th><th className="px-4 py-3">Time</th><th className="px-4 py-3">Created On</th><th className="px-4 py-3"><span className="sr-only">Actions</span></th></tr></thead><tbody className="divide-y divide-slate-200">{entries.map((entry) => <tr key={entry.id} className="hover:bg-slate-50"><td className="px-4 py-4 font-medium text-slate-950">{entry.toolName}</td><td className="px-4 py-4 text-slate-600">{entry.user.name}</td><td className="px-4 py-4 text-slate-600">{entry.category}</td><td className="px-4 py-4 text-slate-600"><div className="whitespace-pre-line">{entry.purpose}</div>{entry.outcome ? <div className="mt-2 whitespace-pre-line text-xs text-slate-500">{entry.outcome}</div> : null}</td><td className="px-4 py-4 text-slate-600">{entry.timeSpent}</td><td className="px-4 py-4 text-slate-600"><div>{dateFormatter.format(entry.createdAt)}</div>{entry.updatedAt.getTime() !== entry.createdAt.getTime() ? <div className="mt-1 text-xs text-slate-500">Edited on {dateFormatter.format(entry.updatedAt)}</div> : null}</td><td className="px-4 py-4">{entry.user.id === sessionUser.id ? <ActionMenu><Link href={`/tools/${entry.id}/edit`} className="rounded px-3 py-2 text-sm text-[#1F2937] transition hover:bg-[#F3E8FF] hover:text-[#770FC2]">Edit</Link><DeleteMenuAction id={entry.id} action={deleteToolUsage} message="Are you sure you want to delete this tool usage entry?" /></ActionMenu> : <span className="text-sm text-slate-400">View only</span>}</td></tr>)}</tbody></table></div>
          )}
        </section>
