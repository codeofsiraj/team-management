  ...
  return (
    <DashboardLayout>
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        ...
        <form className="grid gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm lg:grid-cols-4"><input name="q" defaultValue={query} placeholder="Search..." className="rounded-md border border-slate-300 px-3 py-2 text-sm lg:col-span-2" /><select name="module" defaultValue={module ?? ""} className="rounded-md border border-slate-300 px-3 py-2 text-sm"><option value="">All modules</option><option value="employees">Employees</option><option value="teams">Teams</option><option value="tasks">Tasks</option><option value="updates">Updates</option><option value="learnings">Learnings</option><option value="tools">Tools</option><option value="research">Research</option><option value="activity">Activity</option></select><select name="role" defaultValue={role ?? ""} disabled={sessionUser.role !== "admin"} className="rounded-md border border-slate-300 px-3 py-2 text-sm"><option value="">All roles</option><option value="admin">Admin</option><option value="manager">Manager</option><option value="member">Member</option></select><input name="dateFrom" type="date" defaultValue={dateFrom ?? ""} className="rounded-md border border-slate-300 px-3 py-2 text-sm" /><input name="dateTo" type="date" defaultValue={dateTo ?? ""} className="rounded-md border border-slate-300 px-3 py-2 text-sm" /><select name="teamId" defaultValue={teamId ?? ""} disabled={sessionUser.role !== "admin"} className="rounded-md border border-slate-300 px-3 py-2 text-sm"><option value="">All teams</option>{teamsForFilter.map((team) => <option key={team.id} value={team.id}>{team.name}</option>)}</select><button className="rounded-md bg-slate-950 px-4 py-2 text-sm font-medium text-white">Search</button></form>
