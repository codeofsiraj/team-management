...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  const [users, teams, tasks, updates, learnings, tools, research, activity] = query
    ? await Promise.all([
        ...
        includeModule("updates") ? prisma.dailyUpdate.findMany({ where: { ...ownedWhere, ...(createdAt ? { createdAt } : {}), workedOn: { contains: query } }, include: { user: true }, take: 5 }) : [],
        ...
        includeModule("research") ? prisma.researchDocument.findMany({ where: { ...ownedWhere, ...(createdAt ? { createdAt } : {}), title: { contains: query } }, include: { user: true }, take: 5 }) : [],
  ...
  return (
    <DashboardLayout>
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        ...
        <form className="grid gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm lg:grid-cols-4"><input name="q" defaultValue={query} placeholder="Search..." className="rounded-md border border-slate-300 px-3 py-2 text-sm lg:col-span-2" /><select name="module" defaultValue={module ?? ""} className="rounded-md border border-slate-300 px-3 py-2 text-sm"><option value="">All modules</option><option value="employees">Employees</option><option value="teams">Teams</option><option value="tasks">Tasks</option><option value="updates">Updates</option><option value="learnings">Learnings</option><option value="tools">Tools</option><option value="research">Research</option><option value="activity">Activity</option></select><select name="role" defaultValue={role ?? ""} disabled={sessionUser.role !== "admin"} className="rounded-md border border-slate-300 px-3 py-2 text-sm"><option value="">All roles</option><option value="admin">Admin</option><option value="manager">Manager</option><option value="member">Employee</option></select><input name="dateFrom" type="date" defaultValue={dateFrom ?? ""} className="rounded-md border border-slate-300 px-3 py-2 text-sm" /><input name="dateTo" type="date" defaultValue={dateTo ?? ""} className="rounded-md border border-slate-300 px-3 py-2 text-sm" /><select name="teamId" defaultValue={teamId ?? ""} disabled={sessionUser.role !== "admin"} className="rounded-md border border-slate-300 px-3 py-2 text-sm"><option value="">All teams</option>{teamsForFilter.map((team) => <option key={team.id} value={team.id}>{team.name}</option>)}</select><button className="rounded-md bg-slate-950 px-4 py-2 text-sm font-medium text-white">Search</button></form>
        {!query ? <section className="rounded-lg border border-dashed border-slate-300 bg-white p-8 text-center text-sm text-slate-500">Enter a search term to find records.</section> : <div className="grid gap-4 lg:grid-cols-2"><ResultSection title="Employees" items={users.map((u) => ({ id: u.id, label: u.name, href: "/employees", description: u.email }))} /><ResultSection title="Teams" items={teams.map((t) => ({ id: t.id, label: t.name, href: "/teams", description: t.description ?? undefined }))} /><ResultSection title="Tasks" items={tasks.map((t) => ({ id: t.id, label: t.title, href: "/tasks", description: t.status }))} /><ResultSection title="Updates" items={updates.map((u) => ({ id: u.id, label: u.workedOn, href: "/updates", description: u.user.name }))} /><ResultSection title="Learnings" items={learnings.map((l) => ({ id: l.id, label: l.title, href: "/learnings", description: l.user.name }))} /><ResultSection title="Tools" items={tools.map((t) => ({ id: t.id, label: t.toolName, href: "/tools", description: t.user.name }))} /><ResultSection title="Research" items={research.map((r) => ({ id: r.id, label: r.title, href: "/research", description: r.user.name }))} /><ResultSection title="Activity" items={activity.map((a) => ({ id: a.id, label: a.description, href: "/activity", description: a.user.name }))} /></div>}
      </div>
    </DashboardLayout>
  );
}
